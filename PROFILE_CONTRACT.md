...
## 2026-08-23: comm-board physical link (NEW, non-authentic transport)

Request: add real multi-cabinet networking to the s32comm share-RAM window
(0x800000-0x800FFF, CN/FG at 0x801000/2) for a future Rad Mobile/Rad Rally
twin-cabinet setup -- standalone-vs-network, master privileges, cabinet ID.

Finding first: MAME's own `s32comm` is a local HLE stub -- it never opens a
socket or a serial port, it always reports link-not-connected. There is no
MAME reference behaviour to match. The only hardware evidence found is the
physical "System 32 Multi COMM board" (837-8792), which links two cabinets
with a point-to-point serial cable via CN8 (TX)/CN9 (RX). No DIP-switch or
register-level evidence for master/slave or cabinet-ID semantics was found;
those roles are this session's own design, not a reverse-engineered fact.

Chosen transport (explicit user decision over two alternatives -- a TCP-style
MAME-alike network, which would need HPS/Linux changes in `Main_MiSTer`
outside this repo, and a same-core virtual-link simulation): a plain 3.3V
UART cable between two DE10-Nano USER_IO headers, implemented entirely in
this core's RTL. USER_OUT[0]/USER_IN[0] now carry TXD/RXD (previously
`USER_OUT` was tied `7'h7f`, unused).

New file `rtl/io/s32_comm_link.sv` replaces the disconnected-link stub inside
`rtl/s32_core.sv`'s `S32_UNIVERSAL_DISABLED`-gated IO-7 block (that macro is
not defined by any shipped QSF, so this module compiles into the production
OutRunners image too, inert by default). It keeps CN/FG exactly as before
(each board still reads only its own registers, matching MAME's per-board
semantics) and only replaces the share-RAM backing store: every local write
is queued and streamed to the peer over the UART; every valid inbound packet
is applied at the same address locally. `link_master` only matters at
link-establishment, pushing one full 2048-byte snapshot to the peer so a
late-joining or just-reset board converges instead of racing; after that
sweep both sides are symmetric. `cabinet_id` (0-3) rides in the heartbeat and
is also exposed, non-authentically, at a new 0x801004 read-only byte that no
stock ROM queries.

New OSD entries reuse previously-unallocated status bits (see the
status[8]/[29]/[30-34]/[36-37] convention comment already in
`Arcade-SegaSystem32Multi.sv`): `O[8]` Cabinet Link (Standalone/Network,
default Standalone -- byte-identical to the old stub), `O[30]` Link Role
(Slave/Master), `O[32:31]` Cabinet ID (0-3).

Two bugs were caught and fixed in this session's own manual review (no
Verilog simulator was reachable in that sandbox -- no package-registry
network access -- so this was traced by hand, not tool-verified): the UART
receiver's bit counter started one sample too high and would have shifted
data bit 0 out of the byte on every received byte; and `full_sync_active` /
`full_sync_ctr` were originally written from two separate `always` blocks,
an illegal multi-driver conflict that Quartus would have rejected at
synthesis. Both are fixed in the delivered source (single always block now
owns the full-sync state; RX bit counter starts at 7).

### Follow-up same day: behavioural-model verification found a third bug

No Verilog simulator was reachable (confirmed again: `apt-get`/`pip` network
access is restricted to an allowlist that excludes package registries, and
GitHub archive/raw downloads return `host_not_allowed`/403 outside a
repo explicitly attached to the session). Rather than ship the two
hand-found fixes unverified, this session wrote a Python, cycle-by-cycle
transcription of `s32_comm_link.sv`'s state machines
(`verif/behavioral_model/`) and drove it the way `clk_sys` would.

That model caught a third, real bug the manual read had missed: the RX
engine re-armed start-bit detection immediately after decoding a byte's 8th
data bit, before the actual stop bit had elapsed on the wire. Whenever a
byte's last (MSB) bit was 0, the line was still logically low at that exact
instant, and the very next cycle's idle-search misread the tail of the SAME
byte as a brand-new start bit, corrupting every following byte in a burst.
This only showed up once the model exercised back-to-back multi-byte
transfers; a single isolated byte round-tripped fine, which is exactly why
reading the code by eye missed it. Fixed by giving the RX engine an explicit
third phase (IDLE/DATA/STOP) that waits out one full extra bit period after
decoding before it is allowed to search for a new start bit again.

With that fix, the model's four scenarios all pass: bidirectional dirty-byte
propagation between two boards at a moderate write rate (0/57 addresses
mismatched); the master's full 2048-byte sweep converging a peer that was
blank/late to join (0/2048 mismatched); a clean single write arriving intact
while a deliberately bit-corrupted packet is dropped by the checksum instead
of being applied (share RAM byte stays untouched, not garbage); and a burst
of writes overflowing a deliberately tiny FIFO, recovering to full
consistency once the overflow-triggered master resync completes (0/30
mismatched). A separate, deliberately wrong test run (too high a write rate
for the channel's throughput in the time given, and a full-sync cycle
budget too short for a 2048-byte sweep to finish) reproduced large mismatch
counts and was diagnosed as a test-harness sizing error, not a new RTL bug,
before being corrected -- recorded here so a future chat does not mistake a
similar-looking failure for a real regression without first checking the
test's own cycle/rate budget.

`verif/behavioral_model/README.md` states plainly what this class of test
can and cannot prove: it validates the transcribed *algorithm*, not that
`s32_comm_link.sv` itself compiles, synthesizes, or matches in real
hardware -- a hand transcription can diverge from the actual Verilog. Only
an actual Verilator/Icarus run against the real `.sv`, a Quartus build, and
a real two-board hardware test close that gap; none of those were possible
in this session.

Explicitly NOT done, and NOT verified: no Verilator run against the actual
SystemVerilog, no Quartus build, no two-board hardware bring-up over a real
cable. Treat `rtl/io/s32_comm_link.sv` as untested at the RTL level until run
through this project's own Verilator flow and confirmed on hardware.

### Follow-up: real Verilator testbench added (not run by this session)

`verif/common/tb_comm_link.sv` instantiates two copies of
`s32_comm_link` (standing in for two boards), wires their TXD/RXD both ways,
and runs the same four scenarios as the Python behavioural model directly
against the real SystemVerilog: bidirectional dirty-byte propagation (with
delivery checked on the receiving board, not the one that wrote it locally,
so the check cannot pass trivially with a dead link), the master's full
2048-byte sweep converging a peer preloaded before the link came up, a clean
write versus a deliberately bit-corrupted one (checksum must reject it), and
a 40-write burst against the 32-entry dirty FIFO recovering via the
overflow-triggered resync. Build/run command is in the file's header
comment; Verilator was not available in the session that wrote it (see the
2026-08-23 entries above), so it has NOT been compiled or run against the
real `.sv` -- only reviewed by eye. Run it and report the output (or any
compile error) before trusting `s32_comm_link.sv` at the RTL level. Also
explicitly NOT done: standing up an actual Rad Mobile/Rad Rally build
target. This zip's QSF is pinned `S32_OUTRUNNERS=1` and `releases/` ships
only OutRunners MRA/RBF; `rtl/s32_core.sv`/`s32_pkg.sv` already carry
descriptor-driven Rad Mobile/Rad Rally support from the universal profile,
but a non-OutRunners build profile (its own QSF, its own MRAs) is a
separate, larger task this project's own rules gate behind explicit
authorization -- not started here.

### Follow-up: real Verilator run against the actual RTL -- ALL TESTS PASSED

The user built and ran `verif/common/tb_comm_link.sv` on their own machine
(Verilator 5.050, Apple Silicon/Homebrew, `verilator --binary --timing`) --
the first time `s32_comm_link.sv` was exercised by an actual HDL simulator
rather than the Python behavioural model above. This closes the gap the
previous entry flagged as open.

Getting there required three more fixes, all on the sandbox/toolchain and
testbench side, not the RTL: (1) a `%Error-BADVLTPRAGMA` build failure --
Verilator treats any comment whose first token is literally "verilator"
(case-insensitive) as a pragma directive, and the testbench's own header
comment tripped this; reworded to never start a comment line with that
word. (2) GNU Make (used by `verilator --binary`) cannot build inside a
path containing spaces -- the user's downloaded folder was
`s32multi-main 4` (macOS auto-numbering from repeated downloads); resolved
by a clean re-download to a no-space path. (3) A `%Error: syntax error,
unexpected STRING` from a debug `$display` that relied on C-style adjacent
string-literal concatenation, which SystemVerilog does not do; fixed by
merging the two literals into one.

After those fixes the build succeeded but `test3b` (corrupted-packet
rejection) failed on the first two runs, with the corrupted byte
incorrectly ending up applied. This was NOT an RTL bug: debug
instrumentation (`$display` of `dutB.rx_valid/rx_byte/rp_state` and
`dutA.tx_busy/tp_state` around the corruption window) showed the
testbench's own cable-corruption tap was wired to the wrong direction --
`corrupt_ab` was flipping `a_rxd` (the B-to-A wire) while the test writes
from board A and checks delivery on board B, i.e. the wire that actually
needed corrupting was `b_rxd` (A-to-B). The real link was therefore always
clean regardless of the corruption flag, and the test passed for the wrong
reason. Fixed by swapping which wire `corrupt_ab` muxes onto (`a_rxd =
b_txd` unconditionally; `b_rxd = corrupt_ab ? ~a_txd : a_txd`). After the
fix, `dutB` visibly received the corrupted byte (`rx_byte=0xa4` for a sent
`0xa5`) and correctly rejected it on checksum -- confirming `test3b` now
exercises what it claims to.

With that testbench fix, a clean run against the real `s32_comm_link.sv`
(production parameters, `CLK_HZ=48_317_307`, `BAUD=1_000_000`) reported
**ALL TESTS PASSED** across all four scenarios (bidirectional dirty-byte
propagation with origin tracking, master full-sync sweep, clean write +
corrupted-packet rejection, FIFO-overflow recovery) -- 40+ individual
checks, `$finish` at 103ms simulated time.

This confirms the three RTL bugs found and fixed earlier (RX bit-count
off-by-one, illegal multi-driver on `full_sync_active`/`full_sync_ctr`,
premature RX re-arm before the stop bit) are genuinely fixed in the real
SystemVerilog, not just in the Python transcription. Still NOT done: a
Quartus build of an actual `.rbf`, and a real two-board hardware
bring-up over a physical cable. Both remain open, and the separate
...
**2026-08-23 update -- compile outcome.** A full CI run (`quartus_sh --flow
compile`, ~39 minutes) with this reconstruction completed Analysis &
Synthesis with 0 errors, explicitly elaborating the `pll` / `pll_0002` /
`altera_pll` hierarchy with every derived parameter read back exactly as
written (M=201 as hi=101/lo=100, N=13 as hi=7/lo=6, C0=8/C1=16/C2=8(phase
preset)/C3=32, `pll_fractional_division="0"`,
`pll_output_clk_frequency="773.076923 MHz"`). The Fitter then completed with
0 errors (one retry, 2 originally-unroutable signals resolved automatically;
final fit 79,062 logic cells / 2,132 RAM segments / 1 PLL / 59 DSP on a
...
-0.284 ns (Slow 1100mV 100C corner) and -0.067 ns (Slow 1100mV -40C corner),
both on `emu|pll|pll_inst|altera_pll_i|cyclonev_pll|counter[0].output_counter|divclk`
-- i.e. this PLL's own outclk0 (clk_ram, which also drives SDRAM_CLK via
outclk2). This is a real, if small, timing closure failure and is a
concrete, actionable lead for anyone who sees SDRAM-related instability on
real hardware: it likely means the `pll_cp_current`/`pll_bwctrl` analog
tuning parameters carried over from `pll_hdmi` (see above -- tuned for a
much lower 445.5 MHz VCO, not this PLL's 773 MHz VCO) need retuning, or
that a normal Fitter timing-closure pass (seed sweep, tighter placement
constraints) is needed on this clock domain. Not yet fixed.

Explicitly NOT done, and NOT verified: this PLL reconstruction has NOT been
tested on real DE10-Nano hardware, the timing violation above has NOT been
resolved, and the exact hardware correctness of the M/N/C
reverse-engineering -- while internally consistent, cross-checked against a
real working example, and now also accepted end-to-end by Quartus -- has not
been confirmed by Intel/Altera's own Cyclone V fPLL documentation. Treat
every clock this PLL drives (video timing, SDRAM interface, the V60/V25 CPU
compute domains, and by extension the comm-link's own `CLK_HZ` assumption)
as unverified until a real board boot is confirmed. If the resulting core
visibly misbehaves (no video, SDRAM errors, audio pitch wrong), this PLL
reconstruction -- not the comm-link RTL, which was independently verified
by Verilator earlier -- is the first place to suspect, and the timing
violation above is the most likely specific cause.

## 2026-08-23: TimeQuest report script placeholder (missing file, unrelated to PLL)

The same CI run above still failed overall (`quartus_sh --flow compile`
exit code 3) even after Analysis & Synthesis, Fitter, and Assembler all
succeeded and the `.rbf` had already been generated by the Assembler. The
actual failure was in the TimeQuest Timing Analyzer stage:
`Error: File not found: tools/quartus_report_top_paths.tcl`.

Root cause: `Arcade-SegaSystem32Multi.qsf` sets
`TIMEQUEST_REPORT_SCRIPT tools/quartus_report_top_paths.tcl`, but neither
that file nor even the `tools/` directory existed in this checkout -- the
same class of bug as the PLL gap above (a file the `.qsf`/`.qip` expects
that was simply never committed), this time not hidden by `.gitignore` but
just missing outright.

No original copy of this script was available in this session, so its
exact custom reporting logic could not be recovered. `tools/quartus_report_top_paths.tcl`
was hand-written as a placeholder: for every clock in the design it writes
the top 5 setup and top 5 hold failing paths to `.rpt` files. This runs
*after* Quartus's own standard TimeQuest analysis (the one that already
produced the timing numbers above) and only adds extra report files -- it
does not change synthesis, fitting, or the resulting `.rbf` in any way, so
it carries none of the hardware-correctness risk that the PLL reconstruction
does. It is a reporting-convenience placeholder, not a functional gap.
