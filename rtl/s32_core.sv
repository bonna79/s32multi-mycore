...
`endif
    ...
    // Comm-board physical link (NEW transport, see s32_comm_link.sv header).
    // link_enable=0 keeps the exact pre-existing standalone/disconnected
    // behaviour; this is the OSD default.
    input             comm_link_enable,
    input             comm_link_master,
    input       [1:0] comm_cabinet_id,
    input             comm_link_rxd,
...
// NEW, non-authentic 4-byte id window at 0x801004 (see s32_comm_link.sv
// header): not part of the evidenced register map, stock ROMs do not read
// it. Placed one word above FG (0x801002) so it does not disturb the
// evidenced CN/FG decode below.
wire       sel_comm_id  = sel_comm && (A[15:1] == 15'h0802);
`ifdef S32_UNIVERSAL_DISABLED
...
s32_comm_link comm_link (
    .clk_sys        (clk_sys),
    .rst            (rst),
    .cpu_we_ram     (comm_cpu_we),
    .cpu_addr       (A[11:1]),
    .cpu_wdata      (m_wdata[7:0]),
    .comm_q         (comm_q),
    .cabinet_id_q   (comm_cabinet_id_q),
            ...
            casez (1'b1)
                ...
                sel_comm:    begin
                                if (sel_comm_ram)
                                    ...
                                    // NEW, non-authentic: not part of the
                                    // evidenced s32comm map, see
                                    // s32_comm_link.sv header.
                                    rmux <= {8'hff, comm_cabinet_id_q};
                                else
                                    rmux <= 16'hffff;
                             end
